...
from lib.recons import ReConX1, ReConX2, ReConX3
EPSILON = 1E-9
DEBUG_MODE = False
...
class EnFeaCL(LossTerm):
    ...
    def __init__(self, cfg):
        ...
        if cfg.negative_samples_ratio == -1:
            ...
            self.recon1 = ReConX1(256).cuda()
            self.recon2 = ReConX2(256).cuda()
            self.recon3 = ReConX3(256).cuda()
            self.eye = th.eye(cfg.n_clusters, device=config.DEVICE)
    ...
    def _loss_with_negative_sampling(self, net, cfg, extra):
        ...
        ali_loss = th.nn.functional.cross_entropy(inputs, labels)
        recon_loss = F.mse_loss(self.recon1(net.z1).cuda(), net.view1.cuda()).cuda() + F.mse_loss(
            self.recon2(net.z2).cuda(), net.view2.cuda()).cuda()+ F.mse_loss(
            self.recon3(net.z3).cuda(), net.view3.cuda()).cuda()
        loss = (cfg.c1 * ali_loss) + 0.00001 * recon_loss
        return cfg.delta * loss

    def _loss_without_negative_sampling(self, net, cfg, extra):

        assert len(net.backbone_outputs) == 3, "Contrastive loss without negative sampling only supports 2 views."
...
class EnCluCL(LossTerm):
    ...
    def __call__(self, net, cfg, extra):
        loss_device = th.device("cuda")
        criterion_cluster = contrastive_loss.ClusterLoss(7, 1.0, loss_device).to(
            loss_device)
        loss_cluster01 = criterion_cluster(net.backbone_ddc01, net.output)
        loss_cluster02 = criterion_cluster(net.backbone_ddc02, net.output)
        loss_cluster03 = criterion_cluster(net.backbone_ddc03, net.output)
        loss_cluster = loss_cluster01 + loss_cluster02 + loss_cluster03
        loss = (1-cfg.c1) * loss_cluster
        return loss
